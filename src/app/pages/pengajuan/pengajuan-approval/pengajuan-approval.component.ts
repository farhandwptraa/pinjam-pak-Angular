...
@Component({
  selector: 'app-pengajuan-approval',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './pengajuan-approval.component.html',
})
export class PengajuanApprovalComponent implements OnInit {
  ...
  openReviewModal(pengajuan: PengajuanPendingResponseDTO): void {
    if (pengajuan) {
      ...
      this.catatanMarketing = pengajuan.catatanMarketing || '';
  ...
  submitReview(disetujui: boolean): void {
    ...
    const catatan = this.catatanMarketing.trim();
